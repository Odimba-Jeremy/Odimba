...
def get_patient_queue():
    role = g.current_user.get("role")
    rows = supabase.table("patient_queue").select("*").order("arrival_order").execute().data or []
...
def workflow_consultations():
    ...
    consultation_fee = to_float(data.get("consultation_fee"), 0)
...
def create_final_invoice_from_account():
    ...
    for line in lines:
        if line.get("id"):
            supabase.table("patient_account_lines").update({"status": "invoiced", "invoice_id": result.data[0].get("id"), "updated_at": now_iso()}).eq("id", line["id"]).execute()
    add_audit("CREATE", "billing", f"Facture finale patient #{patient_id}: {total}", result.data[0].get("id"))
    invalidate_cache()
...
def workflow_hospitalizations():
    ...
    payload = {
        ...
        "status": "hospitalized",
        "reason": data.get("reason", ""),
        "daily_rate": to_float(data.get("daily_rate"), 0),
        "created_by": g.current_user["id"],
        "created_by_name": g.current_user["name"],
        "created_at": now_iso(),
        "updated_at": now_iso()
