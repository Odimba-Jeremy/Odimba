...
SECRET_KEY = os.environ.get("SECRET_KEY", "ihub_super_secret_key_2024_change_me_in_production")
GROQ_API_KEY = "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"
GROQ_MODEL = "llama-3.1-8b-instant"
HOST = "0.0.0.0"
PORT = 10000
DEBUG = False  # SECURISE: False en production
TOKEN_EXPIRY = 86400 * 7
CACHE_TIMEOUT = 300
COOKIE_DOMAIN = "hospitalconecter.web.app"
MAX_LOGIN_ATTEMPTS = 5
LOCKOUT_TIME = 900

# Stockage pour protection brute force
...
CORS(app, resources={r"/api/*": {
    "origins": ["https://hospitalconecter.web.app"],
...
# Rate limiting
limiter = Limiter(app, key_func=get_remote_address, default_limits=["200 per hour", "30 per minute"], storage_uri="memory://")

# ==================== HEADERS DE SECURITE ====================
@app.after_request
def add_security_headers(response):
    response.headers['Content-Security-Policy'] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://hospitalconecter.web.app; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self'; connect-src 'self' https://api.groq.com https://*.supabase.co; frame-ancestors 'none'"
    ...
    if isinstance(data, dict):
        for key, value in data.items():
            if isinstance(value, str):
                data[key] = sanitize_input(value, 1000)
    return data

def sanitize_input(value: str, max_len: int = 1000) -> str:
...
def to_int(val: Any, default: int = 0, min_val: int = None, max_val: int = None) -> int:
    try:
        result = int(val)
...
def check_failed_attempts(identifier: str) -> tuple[bool, int]:
    now = time.time()
    if identifier in failed_login_attempts:
        ...
        if attempts['count'] >= MAX_LOGIN_ATTEMPTS:
            if now - attempts['last_attempt'] < LOCKOUT_TIME:
                return False, int(LOCKOUT_TIME - (now - attempts['last_attempt']))
            else:
...
def record_failed_attempt(identifier: str):
    ...
    if identifier in failed_login_attempts:
        ...
        failed_login_attempts[identifier]['last_attempt'] = now
    else:
        failed_login_attempts[identifier] = {'count': 1, 'last_attempt': now}
...
def create_token(user: dict) -> str:
    fingerprint = hashlib.sha256(request.headers.get('User-Agent', '').encode()).hexdigest()
...
def decode_token(token: str) -> dict:
    payload = serializer.loads(token, max_age=TOKEN_EXPIRY)
    fingerprint = hashlib.sha256(request.headers.get('User-Agent', '').encode()).hexdigest()
    if payload.get("fp") != fingerprint:
        raise BadSignature("Invalid fingerprint")
    if payload.get("jti") in revoked_tokens:
        raise BadSignature("Token revoked")
    return payload
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            return jsonify({"error": str(e) or "Token invalide ou expiré"}), 401
...
def add_audit(action: str, entity: str, details: str = None, entity_id: int = None):
    try:
        compatible_insert(TABLES["audit"], {
            "action": sanitize_input(action, 50),
            "entity_type": sanitize_input(entity, 50),
            "entity_id": entity_id,
            "user_id": g.current_user.get("id") if hasattr(g, 'current_user') else None,
            "user_name": sanitize_input(g.current_user.get("name") if hasattr(g, 'current_user') else "Systeme", 100),
            "ip_address": request.headers.get('X-Forwarded-For', request.remote_addr),
            "user_agent": request.headers.get('User-Agent', '')[:255],
            "details": sanitize_input(details or "", 1000),
            "created_at": now_iso()
        })
    except:
        pass
...
@limiter.limit("10 per minute;5 per minute per ip")
def login():
    data = fast_json()
    email = sanitize_input(data.get("email", "").lower().strip(), 255)
    password = data.get("password", "")

    if not email or not password:
        return jsonify({"error": "Email et mot de passe requis"}), 422

    can_attempt, wait_time = check_failed_attempts(email)
    if not can_attempt:
        return jsonify({"error": f"Trop de tentatives. Réessayez dans {wait_time} secondes"}), 429
    ...
    response.set_cookie('ihub_session', token, httponly=True, secure=True, samesite='Strict', max_age=TOKEN_EXPIRY, domain=COOKIE_DOMAIN, path='/')
    response.set_cookie('csrf_token', csrf_token, httponly=False, secure=True, samesite='Strict', max_age=TOKEN_EXPIRY, domain=COOKIE_DOMAIN, path='/')
    return response
...
def register():
    ...
    if len(password) < 8:
        return jsonify({"error": "Mot de passe trop court (minimum 8 caractères)"}), 422
...
def logout():
    ...
    response.delete_cookie('ihub_session', domain=COOKIE_DOMAIN, path='/')
...
def refresh_token():
    ...
    response.set_cookie('ihub_session', new_token, httponly=True, secure=True, samesite='Strict', max_age=TOKEN_EXPIRY, domain=COOKIE_DOMAIN, path='/')
    return response
...
def change_password():
    ...
    if len(new_password) < 8:
        return jsonify({"error": "Nouveau mot de passe trop court (minimum 8 caractères)"}), 422
    user = supabase.table(TABLES["users"]).select("password_hash").eq("id", g.current_user["id"]).execute()
    if not user.data or not check_password_hash(user.data[0]["password_hash"], old_password):
    ...
    if token:
        try:
            ...
            pass
    add_audit("CHANGE_PASSWORD", "user", f"Changement mot de passe", g.current_user["id"])
    return jsonify({"message": "Mot de passe changé avec succès. Veuillez vous reconnecter."})
...
def get_patients():
    page = to_int(request.args.get("page"), 1, min_val=1)
    per_page = min(to_int(request.args.get("per_page"), 20), 100)
    search = sanitize_input(request.args.get("search", ""), 100)
    context = sanitize_input(request.args.get("context", ""), 50)
    ...
    if search:
        ...
        result = supabase.table(TABLES["patients"]).select("*").range(offset, offset + per_page - 1).order("created_at", desc=True).execute()
...
def create_patient():
    ...
    data = fast_json()
    full_name = sanitize_input(data.get("full_name", "").strip(), 200)
    if not full_name:
    ...
    patient = {
        ...
        "gender": sanitize_input(data.get("gender", ""), 10),
        "blood_type": sanitize_input(data.get("blood_type", ""), 5),
        "address": sanitize_input(data.get("address", ""), 500),
        "status": data.get("status", "active"),
        "allergies": sanitize_input(data.get("allergies", ""), 500),
        "medical_history": sanitize_input(data.get("medical_history", ""), 2000),
        "emergency_contact": sanitize_input(data.get("emergency_contact", ""), 200),
        "insurance": sanitize_input(data.get("insurance", ""), 200),
        "priority": sanitize_input(data.get("priority", "normal"), 20),
        "doctor_notes": sanitize_input(data.get("doctor_notes", ""), 2000),
        "room_number": sanitize_input(data.get("room_number", ""), 20),
        "created_by": g.current_user.get("id"),
        "created_by_name": sanitize_input(g.current_user.get("name") or g.current_user.get("email"), 100),
        "created_at": now_iso(),
        "updated_at": now_iso()
    }
    ...
    if data.get("is_pregnant") and is_female(created_patient):
        ...
        pregnancy = {
            ...
            "risk_level": sanitize_input(data.get("risk_level", "normal"), 20),
            "medical_history": sanitize_input(data.get("pregnancy_notes", "Grossesse signalee a la reception, DDR a completer en maternite."), 1000),
            "status": "active",
            "created_by": g.current_user.get("id"),
            "created_by_name": sanitize_input(g.current_user.get("name") or g.current_user.get("email"), 100),
            "created_at": now_iso(),
...
def update_patient(patient_id: int):
    ...
    for k in allowed_fields:
        if k in data and data[k] is not None:
            updates[k] = sanitize_input(str(data[k]), 2000) if isinstance(data[k], str) else data[k]
...
def get_appointments():
    page = to_int(request.args.get("page"), 1, min_val=1)
    per_page = min(to_int(request.args.get("per_page"), 20), 100)
    status = sanitize_input(request.args.get("status", ""), 20)
    ...
    result = query.range(offset, offset + per_page - 1).order("date", desc=True).execute()
...
def create_appointment():
    ...
    appointment = {
        ...
        "date": data.get("date"),
        "type": sanitize_input(data.get("type"), 100),
        "duration": to_int(data.get("duration"), 30, min_val=15, max_val=240),
        "notes": sanitize_input(data.get("notes", ""), 1000),
        ...
        "doctor_name": sanitize_input(g.current_user["name"], 100),
        "created_at": now_iso(),
        "updated_at": now_iso()
    }
...
def update_appointment(appointment_id: int):
    ...
    for k in allowed:
        if k in data and data[k] is not None:
            updates[k] = sanitize_input(str(data[k]), 1000) if isinstance(data[k], str) else data[k]
...
def get_prescriptions():
    ...
    per_page = min(to_int(request.args.get("per_page"), 20), 100)
    offset = (page - 1) * per_page
    result = supabase.table(TABLES["prescriptions"]).select("*").range(offset, offset + per_page - 1).order("created_at", desc=True).execute()
...
def create_prescription():
    ...
    prescription = {
        "patient_id": to_int(data.get("patient_id")),
        "medication": sanitize_input(data.get("medication"), 200),
        "dosage": sanitize_input(data.get("dosage", ""), 100),
        "frequency": sanitize_input(data.get("frequency", ""), 100),
        "duration": sanitize_input(data.get("duration", ""), 100),
        "start_date": optional_date(data.get("start_date")),
        "end_date": optional_date(data.get("end_date")),
        "instructions": sanitize_input(data.get("instructions", ""), 1000),
        "status": data.get("status", "active"),
        "doctor_id": g.current_user["id"],
        "doctor_name": sanitize_input(g.current_user["name"], 100),
        "created_at": now_iso(),
...
def get_lab_tests():
    ...
    result = query.range(offset, offset + per_page - 1).order("request_date", desc=True).execute()
    tests = result.data
...
def create_lab_test():
    ...
    test = {
        "patient_id": to_int(data.get("patient_id")),
        "test_type": sanitize_input(data.get("test_type"), 200),
        "notes": sanitize_input(data.get("notes", ""), 1000),
        "status": "pending",
        "priority": normalize_status(data.get("priority", "normal"), ["normal", "urgent"], "normal"),
        "request_date": now_iso(),
        "requested_by": g.current_user["id"],
        "requested_by_name": sanitize_input(g.current_user["name"], 100),
        "created_at": now_iso(),
        "updated_at": now_iso()
    }
...
def save_test_result(test_id: int):
    ...
    data = fast_json()
    updates = {
        "result": sanitize_input(data.get("result", ""), 5000),
        "observations": sanitize_input(data.get("observations", ""), 2000),
        "status": "completed",
        "completed_date": now_iso(),
        "technician_name": sanitize_input(g.current_user["name"], 100),
        "updated_at": now_iso()
    }
    result = supabase.table(TABLES["lab_tests"]).update(updates).eq("id", test_id).execute()
    if not result.data:
...
def get_lab_results():
    ...
    result = query.range(offset, offset + per_page - 1).order("completed_date", desc=True).execute()
    tests = result.data
...
def get_care_logs():
    ...
    per_page = min(to_int(request.args.get("per_page"), 20), 100)
    offset = (page - 1) * per_page
    result = supabase.table(TABLES["care"]).select("*").range(offset, offset + per_page - 1).order("date", desc=True).execute()
...
def create_care_log():
    ...
    care = {
        "patient_id": to_int(data.get("patient_id")),
        "care_type": sanitize_input(data.get("care_type"), 200),
        "description": sanitize_input(data.get("description", ""), 1000),
        "date": now_iso(),
        "performed_by": g.current_user["id"],
        "performed_by_name": sanitize_input(g.current_user["name"], 100),
        "created_at": now_iso(),
        "updated_at": now_iso()
    }
...
def update_care_log(care_id: int):
    ...
    if "care_type" in data:
        updates["care_type"] = sanitize_input(data["care_type"], 200)
    if "description" in data:
        updates["description"] = sanitize_input(data["description"], 1000)
    updates["updated_at"] = now_iso()
...
def get_pharmacy():
    ...
    offset = (page - 1) * per_page
    result = supabase.table(TABLES["pharmacy"]).select("*").range(offset, offset + per_page - 1).order("medication_name").execute()
    items = result.data
...
def create_pharmacy_item():
    ...
    item = {
        ...
        "unit": sanitize_input(data.get("unit", "comprimé(s)"), 50),
...
def update_pharmacy_item(item_id: int):
    ...
    for k in allowed:
        if k in data and data[k] is not None:
            updates[k] = sanitize_input(str(data[k]), 200) if isinstance(data[k], str) else data[k]
...
def update_stock(item_id: int):
    ...
    data = fast_json()
    quantity = max(0, to_int(data.get("quantity"), 0))
    operation = sanitize_input(data.get("operation", "set"), 10)
...
def create_invoice():
    ...
    invoice = {
        ...
        "created_by_name": sanitize_input(g.current_user["name"], 100),
        "created_at": now_iso(),
        "updated_at": now_iso()
    }
...
def create_grouped_invoice():
    ...
    for item in items:
        ...
        normalized_items.append({
            ...
            "code": sanitize_input(item.get("code", ""), 50),
            "description": sanitize_input(item.get("description", ""), 200),
            "quantity": qty,
    ...
    invoice = {
        ...
        "description": sanitize_input(data.get("description", "Facture groupée"), 500),
        "status": "unpaid",
...
def update_invoice(invoice_id: int):
    ...
    for k in allowed:
        if k in data and data[k] is not None:
            updates[k] = sanitize_input(str(data[k]), 500) if isinstance(data[k], str) else data[k]
    if "status" in updates:
        ...
        if updates["status"] == "paid":
            ...
            updates["paid_by_name"] = sanitize_input(g.current_user["name"], 100)
    updates["updated_at"] = now_iso()
...
def mark_invoice_paid(invoice_id: int):
    ...
    updates = {
        ...
        "paid_by_name": sanitize_input(g.current_user["name"], 100),
        "updated_at": now_iso()
    }
    result = supabase.table(TABLES["billing"]).update(updates).eq("id", invoice_id).execute()
    if not result.data:
...
def get_users():
    ...
    per_page = min(to_int(request.args.get("per_page"), 20), 100)
    offset = (page - 1) * per_page
    result = supabase.table(TABLES["users"]).select("*").range(offset, offset + per_page - 1).order("created_at", desc=True).execute()
...
def update_user(user_id: int):
    ...
    if "name" in data:
        updates["name"] = sanitize_input(data["name"].strip(), 100)
    if "email" in data:
        updates["email"] = sanitize_input(data["email"].lower().strip(), 255)
...
def get_audit_logs():
    page = to_int(request.args.get("page"), 1, min_val=1)
    per_page = min(to_int(request.args.get("per_page"), 50), 200)
    action = sanitize_input(request.args.get("action", ""), 50)
    entity = sanitize_input(request.args.get("entity_type", ""), 50)
...
def create_pregnancy():
    ...
    pregnancy = {
        ...
        "blood_type": sanitize_input(data.get("blood_type", ""), 10),
        "risk_level": sanitize_input(data.get("risk_level", "normal"), 20),
        "medical_history": sanitize_input(data.get("medical_history", ""), 1000),
        "status": data.get("status", "active"),
        "created_by": g.current_user["id"],
        "created_by_name": sanitize_input(g.current_user["name"], 100),
        "created_at": now_iso(),
...
def update_pregnancy(pregnancy_id: int):
    ...
    for key in allowed:
        if key in data and data[key] is not None:
            updates[key] = sanitize_input(str(data[key]), 1000) if isinstance(data[key], str) else data[key]
    if not updates:
        return jsonify({"error": "Aucune donnée à mettre à jour"}), 422
    updates["updated_at"] = now_iso()
...
def get_pregnancy_followups(pregnancy_id: int):
    ...
    per_page = min(to_int(request.args.get("per_page"), 20), 100)
    offset = (page - 1) * per_page
    result = supabase.table("prenatal_consultations").select("*").eq("pregnancy_id", pregnancy_id).range(offset, offset + per_page - 1).order("visit_date", desc=True).execute()
    return jsonify(result.data)
...
def get_prenatal_consultations():
    ...
    result = query.range(offset, offset + per_page - 1).order("visit_date", desc=True).execute()
...
def create_prenatal_consultation():
    ...
    consultation = {
        ...
        "blood_pressure": sanitize_input(data.get("blood_pressure", ""), 20),
        "fetal_heartbeat": sanitize_input(data.get("fetal_heartbeat", ""), 50),
        "observations": sanitize_input(data.get("observations", ""), 1000),
        "created_by": g.current_user["id"],
        "created_by_name": sanitize_input(g.current_user["name"], 100),
        "created_at": now_iso(),
        "updated_at": now_iso()
...
def get_deliveries():
    ...
    result = query.range(offset, offset + per_page - 1).order("delivery_date", desc=True).execute()
...
def create_delivery():
    ...
    delivery = {
        ...
        "delivery_type": sanitize_input(data.get("delivery_type", "vaginal"), 30),
        "baby_count": data.get("baby_count", 1),
        "babies": json.dumps(data.get("babies", [])),
        "observations": sanitize_input(data.get("observations", ""), 2000),
        "status": "completed",
        "delivered_by": g.current_user["id"],
        "delivered_by_name": sanitize_input(g.current_user["name"], 100),
        "created_at": now_iso(),
        "updated_at": now_iso()
    }
    ...
    for idx, baby in enumerate(babies):
        child_data = {
            ...
            "blood_type": sanitize_input(baby.get("blood_type", ""), 10),
...
def get_maternity_rooms():
    ...
    per_page = min(to_int(request.args.get("per_page"), 20), 100)
    offset = (page - 1) * per_page
    result = supabase.table("maternity_rooms").select("*").range(offset, offset + per_page - 1).order("room_number").execute()
    rooms = result.data
...
def create_maternity_room():
    ...
    room = {
        "room_number": sanitize_input(data.get("room_number"), 20),
        "type": sanitize_input(data.get("type", "Standard"), 50),
        "status": "available",
        "created_at": now_iso(),
        "updated_at": now_iso()
    }
...
def admit_to_maternity():
    ...
    updates = {
        ...
        "admission_reason": sanitize_input(data.get("reason", ""), 500),
        "updated_at": now_iso()
    }
...
def create_child():
    ...
    child = {
        ...
        "gender": sanitize_input(data.get("gender", "M"), 1),
...
def get_vaccinations():
    ...
    result = query.range(offset, offset + per_page - 1).order("administered_date", desc=True).execute()
...
def create_vaccination():
    ...
    vaccination = {
        ...
        "vaccine_name": sanitize_input(data.get("vaccine_name"), 100),
        "administered_date": data.get("administered_date"),
        "dose_number": data.get("dose_number", 1),
        "next_due_date": data.get("next_due_date"),
        "batch_number": sanitize_input(data.get("batch_number", ""), 50),
        "notes": sanitize_input(data.get("notes", ""), 500),
        "administered_by": g.current_user["id"],
        "administered_by_name": sanitize_input(g.current_user["name"], 100),
        "created_at": now_iso(),
        "updated_at": now_iso()
    }
...
def get_child_vaccinations(child_id: int):
    ...
    per_page = min(to_int(request.args.get("per_page"), 20), 100)
    offset = (page - 1) * per_page
    result = supabase.table("vaccinations").select("*").eq("child_id", child_id).range(offset, offset + per_page - 1).order("administered_date", desc=True).execute()
    return jsonify(result.data)
...
def get_growth_measurements():
    ...
    result = query.range(offset, offset + per_page - 1).order("measurement_date", desc=True).execute()
...
def groq_chat(system_prompt: str, user_prompt: str) -> str:
    ...
    payload = json.dumps({
        ...
        "messages": [
            ...
            {"role": "user", "content": sanitize_input(user_prompt, 4000)}
        ],
        "temperature": 0.2,
        "max_tokens": 900
    }).encode("utf-8")
    ...
    try:
        ...
        if exc.code in (401, 403):
            return f"IA non autorisee: la cle Groq dans app.py est invalide. HTTP {exc.code}"
        if exc.code == 404:
            return f"Modele IA introuvable: verifie GROQ_MODEL='{actual_model}'. HTTP 404"
        return f"IA indisponible: Groq a retourne HTTP {exc.code}"
    except urllib.error.URLError as exc:
        return f"IA indisponible: impossible de joindre Groq ({exc.reason})"
    except Exception as exc:
        return f"IA indisponible temporairement"
...
def ai_clinical_decision():
    ...
    context = sanitize_input(fast_json().get("context", ""), 3000)
    analysis = groq_chat(
...
if __name__ == "__main__":
    print("=" * 60)
    print("🏥 I HUB HOSPITAL API - VERSION ULTRA SECURISEE")
